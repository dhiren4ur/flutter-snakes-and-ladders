...
class _MainMenuScreenState extends State<MainMenuScreen>
  ...
  void _loadBannerAd() {
    ...
    if (!kIsWeb && Platform.isAndroid) {
      print('🎯 Loading banner ad for main menu...');
      AdManager.instance.loadBannerAd(
        onAdLoaded: (ad) {
          if (mounted) {
            setState(() {
              _bannerAd = ad;
              print('✅ Banner ad ready for display');
            });
          }
        },
      );
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              // Banner Ad at bottom (ONLY on Android)
              if (_bannerAd != null && !kIsWeb && Platform.isAndroid)
                Container(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color:
                (isPrimary ? Colors.blue : Colors.grey[800]!).withOpacity(0.3),
            blurRadius: 8,
...
class _RulesScreenState extends State<RulesScreen> {
  ...
  void _loadBannerAd() {
    // Only load ads on Android
    if (!kIsWeb && Platform.isAndroid) {
      print('🎯 Loading banner ad for rules screen...');
      AdManager.instance.loadBannerAd(
        onAdLoaded: (ad) {
          if (mounted) {
            setState(() {
              _bannerAd = ad;
              print('✅ Banner ad ready for rules screen');
            });
          }
        },
      );
